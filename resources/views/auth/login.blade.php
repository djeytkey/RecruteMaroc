<x-guest-layout>
    ...
    @if(session('error'))<p class="mb-4 text-sm text-red-600">{{ session('error') }}</p>@endif

    @php($hasSocial = config('services.google.client_id') || config('services.facebook.client_id'))
    @if($hasSocial)
    ...
    <p class="mb-4 text-center text-sm text-slate-500">— ou —</p>
    @endif
